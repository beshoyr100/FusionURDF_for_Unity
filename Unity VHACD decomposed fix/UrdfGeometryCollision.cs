...
namespace Unity.Robotics.UrdfImporter
{
    public class UrdfGeometryCollision : UrdfGeometry
    {
        ...
        private static GameObject CreateCylinderCollider()
        {
            ...
            Link.Geometry.Cylinder cylinder = new Link.Geometry.Cylinder(0.5, 2);
        ...
        private static void ConvertMeshToColliders(GameObject gameObject, string location = null, bool setConvex = true)
        {
            ...
            else
            {
                ...
                foreach (MeshFilter meshFilter in meshFilters)
                {
                    ...
                    // --- NULL MESH GUARD ---
                    // VHACD crashes if sharedMesh is null. Skip decomposition and add an
                    // empty fallback MeshCollider so the rest of the import can continue.
                    if (meshFilter.sharedMesh == null)
                    {
                        Debug.LogWarning(
                            $"[URDF Importer] Skipping VHACD decomposition for '{child.name}': " +
                            $"sharedMesh is null. A plain MeshCollider will be used as fallback.");
                        child.AddComponent<MeshCollider>(); // empty but won't crash
                        Object.DestroyImmediate(child.GetComponent<MeshRenderer>());
                        Object.DestroyImmediate(meshFilter);
                        continue;
                    }
