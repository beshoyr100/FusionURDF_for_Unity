...
Responsibilities
----------------
* :func:`copy_occs`           — Duplicate Fusion occurrences before OBJ export
* :func:`export_obj`          — Export each occurrence as an OBJ file
...
def copy_occs(root):
    ...
    filtered out during export (see :func:`export_obj`).
    ...
    # Mark originals so export_obj can skip them
    for occ in old_occs:
        try:
            occ.component.name = 'old_component'
        except Exception as e:
...
def export_obj(design, save_dir, robot_name, components):
    """Export each occurrence as an OBJ file into *meshes/*.
    ...
    OBJ files are written to::

        {save_dir}/{robot_name}/meshes/{component_name}.obj
    ...
    for component in components:
        for occ in component.allOccurrences:
            ...
            try:
                opts = export_mgr.createOBJExportOptions(occ, file_path)
    ...
    if failed:
        raise RuntimeError(
            'OBJ export completed with errors.\n\n'
