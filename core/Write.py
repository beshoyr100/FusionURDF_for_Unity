...
def _write_gazebo_section(joints_dict, file_name):
    """Write <gazebo> reference blocks for every link.

    These are kept for completeness / compatibility (they match the example
    output).  Unity ignores them.
    """
    with open(file_name, mode='a') as f:
        f.write('<gazebo>\n')
        f.write('  <plugin name="control" filename="libgazebo_ros_control.so"/>\n')
        f.write('</gazebo>\n')
...
def write_urdf(joints_dict, links_xyz_dict, inertial_dict, robot_name, save_dir):
    ...
    _write_gazebo_section(joints_dict, file_name)
